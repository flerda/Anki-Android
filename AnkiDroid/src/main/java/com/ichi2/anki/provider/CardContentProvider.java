...
public class CardContentProvider extends ContentProvider {
     ...
     */
    private static String[] defaultNoteProjectionDBAccess = FlashCardsContract.Note.DEFAULT_PROJECTION.clone();
    static{
        for(int idx = 0; idx < defaultNoteProjectionDBAccess.length; idx++){
            if(defaultNoteProjectionDBAccess[idx].equals(FlashCardsContract.Note._ID)){
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match){
            case NOTES: {
                ...
                int insertCount = col.addNote(newNote);
...
//                break;
        ...
        switch (match){
            ...
            case MODELS: {
                ...
                for (Long modelId : models.keySet()) {
                    try {
                        addModelToCursor(modelId, models, rv, col, columns);
    ...
    private void addModelToCursor(Long modelId, HashMap<Long, JSONObject> models, MatrixCursor rv, Collection col, String[] columns) throws JSONException {
    ...
    private static String proj2str(String[] projection) {
        ...
        if(projection != null){
            for(String column : projection){
                ...
                if(idx>=0){
                    rv.append(defaultNoteProjectionDBAccess[idx]);
                    rv.append(",");
                }
                else{
            ...
            for(String column : defaultNoteProjectionDBAccess){
                rv.append(column);
                rv.append(",");
            }
        }
...
//                break;
    ...
    Collection getCol(){
        Collection col;
        //Hack to prevent crashes on some devices
        ...
        /* The collection may not have been loaded, because the Application class AnkiAndroidApp
         * will not be created when started by ContentProvider.
         *   "[Application.onCreate is called] when the application is starting, before any
         *    activity, service, or receiver objects (excluding content providers) have been
         *    created."
         * (from http://developer.android.com/reference/android/app/Application.html#onCreate%28%29)
         *
         * Hence the collection is loaded manually if the application is not running.
         */
        if(!AnkiDroidApp.colIsOpen()){
            ...
            try {
                ...
                Timber.e(e, "CardContentProvider.query - RuntimeException on opening collection");
                AnkiDroidApp.sendExceptionReport(e, "CardContentProvider.query");
                return null;
            }
        }
        else{
    ...
    private Card getCardFromUri(Uri uri, Collection col){
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid ord format", e);
        }
